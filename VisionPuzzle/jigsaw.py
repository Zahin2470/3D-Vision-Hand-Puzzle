...
class Piece:
    ...
    # Hard-mode orientation: 0/90/180/270 degrees. A piece only counts
    # as placed once it's both in the right spot *and* rotation == 0.
    rotation: int = 0
...
def _rotate_tile(image: np.ndarray, degrees: int, w: int, h: int) -> np.ndarray:
    """Rotate a tile by a multiple of 90 degrees, keeping its footprint
    (w, h) fixed so board layout and snap math never have to change."""
    degrees %= 360
    if degrees == 0:
        return image
    if degrees == 180:
    ...
    else:  # pragma: no cover - rotation is always a multiple of 90
        return image
    if rotated.shape[1] != w or rotated.shape[0] != h:
...
class JigsawPuzzle:
    ...
    def rotate_held(self, delta: int) -> bool:
        """Rotate whichever piece(s) are currently held by `delta` degrees
        (a multiple of 90). No-op outside Hard mode. Returns True if any
        piece actually rotated."""
        if not self.allow_rotation or not self.held:
            return False
        rotated = False
        for idx in set(self.held.values()):
    ...
    def drop(self, hand_key: str) -> tuple[bool, Optional[tuple[float, float]]]:
        ...
        p.holder = None
        if (
            abs(p.x - p.target_x) <= self.snap_px
            and abs(p.y - p.target_y) <= self.snap_px
            and p.rotation == 0
        ):
            p.x = p.target_x
            p.y = p.target_y
    ...
    def _display_image(self, p: Piece) -> np.ndarray:
        """Piece's current tile image, rotated for Hard mode with a
        small per-piece cache so we only re-rotate on actual change."""
        if p.rotation == 0:
            return p.image
        if p._rot_cache_deg != p.rotation:
            p._rot_cache_img = _rotate_tile(p.image, p.rotation, p.w, p.h)
            p._rot_cache_deg = p.rotation
        return p._rot_cache_img
    ...
    def draw(self, frame: np.ndarray, *, show_guides: bool = True) -> np.ndarray:
        ...
        for p in ordered:
            ...
            if x < 0 or y < 0 or x + p.w > out.shape[1] or y + p.h > out.shape[0]:
                continue
            out[y:y + p.h, x:x + p.w] = self._display_image(p)
            if p.placed:
            ...
            in_slot = abs(p.x - p.target_x) <= self.snap_px and abs(p.y - p.target_y) <= self.snap_px
            if p.holder and in_slot and p.rotation == 0:
                cv2.rectangle(
                    out, (int(p.target_x), int(p.target_y)),
                    (int(p.target_x) + p.w - 1, int(p.target_y) + p.h - 1),
                    ui.SUCCESS, 2, cv2.LINE_AA,
                )
            elif p.rotation != 0 and not p.placed:
                # Rotated but not fixed yet — flag it so the player knows
                # positioning alone won't be enough to snap this piece.
                cv2.rectangle(out, (x, y), (x + p.w - 1, y + p.h - 1), ui.ACCENT_HOT, 1, cv2.LINE_AA)
                ui.put_text(out, f"{p.rotation}\u00b0", (x + 4, y + 16), scale=0.38, color=ui.ACCENT_HOT)
